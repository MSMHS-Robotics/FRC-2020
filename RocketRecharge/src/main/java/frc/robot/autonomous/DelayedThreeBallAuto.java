...
public class DelayedThreeBallAuto extends SequentialCommandGroup {
     ...
     */
    public DelayedThreeBallAuto(Drivetrain drivetrain, Intake intake, Shooter shooter, Limelight limelight) {
        super(new DeployIntakeCommand(intake, 1), // deploy
                new ResetGyroCommand(drivetrain), // fresh start
                new WaitCommand(5), // delay
                new WarmupCommand(shooter, drivetrain, null, 90, true), // warm up
                new WaitCommand(1), // wait
                new ShootBurstCommand(shooter, intake, drivetrain, limelight, null, 90, 5, true), // Shoot!
